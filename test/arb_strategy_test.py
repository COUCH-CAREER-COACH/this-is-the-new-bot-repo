...
@patch('web3.Web3.toWei', lambda _, number, unit='ether': int(float(number) * 10**18))
class TestArbStrategy(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        # Mock Web3 utils
        self.w3.to_wei = lambda number, unit='ether': int(float(number) * 10**18)
    ...
    @patch('src.utils.dex_utils.DEXHandler.decode_swap_data')
    @patch('src.utils.dex_utils.DEXHandler.get_pool_info')
    async def test_analyze_unprofitable_tx(self, mock_get_pool_info, mock_decode_swap):
